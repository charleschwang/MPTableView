        ...
        if (MPTV_IS_STABLE_UPDATE_TYPE(node.type)) {
            ...
            if (node.type == MPTableViewUpdateReload) {
                ...
                if ([_tableView _needsDisplayAtSection:deletedSection withUpdateType:MPTableViewUpdateDelete withOffset:0]) {
                    CGFloat proposedDeletedPositionY = [_tableView _getProposedDeletedPositionY];
                    
                    for (NSInteger j = 0; j < deletedSection.numberOfRows; j++) {
                        [_tableView _deleteCellInSection:node.originalIndex row:j animation:animation sectionPosition:deletedSection proposedDeletedPositionY:proposedDeletedPositionY];
                    }
                    
                    [_tableView _deleteSectionViewInSection:node.originalIndex viewType:MPTableViewSectionHeader animation:animation deletedSection:deletedSection proposedDeletedPositionY:proposedDeletedPositionY];
                    [_tableView _deleteSectionViewInSection:node.originalIndex viewType:MPTableViewSectionFooter animation:animation deletedSection:deletedSection proposedDeletedPositionY:proposedDeletedPositionY];
                }
...
- (NSInteger)rowForPosition:(CGFloat)position {
    if (_headerHeight > 0 && (position <= self.start + _headerHeight)) {
        return MPTableViewSectionHeader;
    }
    if (_footerHeight > 0 && (position >= self.end - _footerHeight)) {
        ...
        if (MPTV_IS_STABLE_UPDATE_TYPE(node.type)) {
            ...
            if (node.type == MPTableViewUpdateReload) {
                ...
                *endPositionIterator = startPosition + height;
                ...
                MPTableViewUpdateNormalNode &normalNode = (MPTableViewUpdateNormalNode &)node;
                MPTableViewRowAnimation animation = normalNode.animation;
                CGFloat proposedDeletedPositionY = [tableView _getProposedDeletedPositionY];
                
                // node.index - step == node.originalIndex
                [tableView _deleteCellInSection:previousSection row:node.originalIndex animation:animation sectionPosition:nil proposedDeletedPositionY:proposedDeletedPositionY];
                if (needsDisplay) {
                    CGFloat proposedInsertedLocationY = [tableView _getProposedInsertedLocationY];
                    NSInteger row = node.index;
                    void (^updateAction)(void) = ^{
                ...
                if (node.type == MPTableViewUpdateDelete) {
                    ...
                    [tableView _deleteCellInSection:previousSection row:node.originalIndex animation:normalNode.animation sectionPosition:nil proposedDeletedPositionY:proposedDeletedPositionY];
                }
...
- (CGFloat)updateForTableView:(MPTableView *)tableView toSection:(NSInteger)newSection withOffset:(CGFloat)offset needsDisplay:(BOOL)needsDisplay {
    BOOL shouldDisplay = (offset != 0) && [tableView _shouldDisplayAtSection:self forRelayoutWithOffset:offset];
    CGFloat previousOffset = offset;
